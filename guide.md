...
11. [Version History & Revert](#11-version-history--revert)
...
## 11. Version History & Revert

As collaborative documents evolve, users often need the ability to view previous versions and restore to earlier states. This section covers how to implement **version history** with a **revert** feature that syncs to all connected users.

### 11.1 How Yjs Versioning Works

Yjs documents inherently contain their full history through the CRDT structure. To implement practical versioning:

1. **Snapshots**: Capture document state at specific points using `Y.encodeStateAsUpdate(doc)`
2. **Storage**: Store encoded snapshots as binary blobs in your database
3. **Restoration**: Apply the "git revert" style — changes are applied as new updates, not document replacement

This approach ensures that when a user reverts to an old version, **all connected users** see the revert happen in real-time via the normal CRDT sync mechanism.

### 11.2 Database Schema

Add a `DocumentVersion` model to store version snapshots:
...
model DocumentVersion {
    ...
    document   Document @relation(fields: [documentId], references: [id], onDelete: Cascade)
    snapshot   Bytes                  // Encoded Yjs document state
...
> **Note**: We store the full document state (`Y.encodeStateAsUpdate`) rather than a lightweight snapshot (`Y.snapshot`) because we need to reconstruct the document content for revert operations.

### 11.3 Creating Versions

**Manual Version Creation (API Endpoint)**
...
export async function POST(request: NextRequest, { params }: RouteParams) {
    ...
    // Get current Y.Doc from memory
    const { docs } = await import('@y/websocket-server/utils');
    const doc = docs.get(documentId);

    if (!doc) {
        return NextResponse.json(
            { error: 'Document not loaded. Open it first.' },
            { status: 404 }
        );
    }
...
**Automatic Version Creation (During Compaction)**

For automatic versioning, hook into the compaction process:

```typescript
// persistence.ts - inside compactDocument()
const AUTO_VERSION_INTERVAL_MS = 60_000; // 60 seconds
...
### 11.4 The Revert Mechanism

Reverting uses a **"git revert" style approach** rather than replacing the document:

```
┌─────────────────────────────────────────────────────────────────┐
│                    REVERT FLOW                                   │
│                                                                  │
│  1. Load target snapshot from database                          │
│                    ↓                                             │
│  2. Create temporary Y.Doc, apply snapshot                      │
│                    ↓                                             │
│  3. Get current live Y.Doc from memory                          │
│                    ↓                                             │
│  4. Within a transaction, clear & rebuild shared types:         │
│     • Y.Map('cia') - clear keys, copy from temp                 │
│     • Y.Map('controls') - clear keys, copy from temp            │
│     • Y.XmlFragment('prosemirror') - clear, clone children      │
│                    ↓                                             │
│  5. Transaction generates updates → broadcasted to all clients  │
│                    ↓                                             │
│  6. All connected users see the reverted content                │
└─────────────────────────────────────────────────────────────────┘
```
...
export async function POST(request: NextRequest, { params }: RouteParams) {
    ...
    // Create temp doc from snapshot
    const Y = await import('yjs');
    const tempDoc = new Y.Doc();
    Y.applyUpdate(tempDoc, new Uint8Array(version.snapshot));

    // Apply revert within a transaction
    currentDoc.transact(() => {
        // Clear and restore each shared type
        const currentCia = currentDoc.getMap('cia');
        const tempCia = tempDoc.getMap('cia');
        
        currentCia.forEach((_, key) => currentCia.delete(key));
        tempCia.forEach((value, key) => currentCia.set(key, value));

        // ... repeat for other shared types (controls, prosemirror)
    }, 'revert');
...
> [!IMPORTANT]
> The revert happens inside a `transact()` call with origin `'revert'`. This generates a single update that is broadcast to all connected clients, ensuring everyone sees the revert simultaneously.

### 11.5 Version History UI

Create a dropdown component that displays version history:

```typescript
// components/VersionHistoryDropdown.tsx
export default function VersionHistoryDropdown({ documentId }: Props) {
    ...
    const handleRevert = async (versionId: number) => {
        if (!confirm('Revert to this version? All users will see the change.')) {
            return;
        }
                ...
                <div className="dropdown-panel">
                    {versions.map(v => (
                        <div key={v.id}>
                            ...
                            <span>{v.label}</span>
                            <button onClick={() => handleRevert(v.id)}>
                                Revert
                            </button>
                        </div>
                    ))}
...
### 11.6 Version Retention

To prevent unbounded storage growth, enforce a maximum version limit:

```typescript
async function enforceVersionLimit(docId: string, maxVersions = 50) {
...
| Practice | Rationale |
|----------|----------|
| Store full state, not snapshots | Easier reconstruction for revert |
| Use transactions for revert | Single update broadcast to all clients |
| Auto-version during compaction | Leverages existing timer infrastructure |
| Limit versions per document | Prevents storage bloat |
| Show confirmation before revert | Prevents accidental data loss |

> [!TIP]
> Consider adding a "Preview" mode that shows the version content in read-only mode before committing to a revert. This can be implemented by creating a temporary `Y.Doc` from the snapshot and rendering it in a separate view.
